...
public class Lir {
	...
	public static void main(String args[]){
		...
		if(lir.m_cmd.hasOption("search")){
			if(!lir.m_cmd.getOptionValue("search").contains(",")) {
				System.out.println("You should provide fieldname,fieldvalue while using search option");
				lir.usage();
			}
			...
			System.out.println("You should use either -listall option or -search option");
			lir.usage();
		}
	}
	...
	private void createOptions(){
		...
		Option indexPath = new Option("i", "indexpath", true, "path to the lucene index directory");
		indexPath.setRequired(true);
		indexPath.setArgs(1);
		indexPath.setArgName("pathToIdxDir");
		m_options.addOption(indexPath);
				
		Option listall = new Option("listall", false, "lists all the documents of the lucene index");
		m_options.addOption(listall);

		Option search = new Option("search", true, "search the index based on fieldname and fieldvalue combination");
		search.setArgs(2);
